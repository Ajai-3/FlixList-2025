...
const MediaMetaInfo: React.FC<MediaDetailsProps> = ({ media }) => {
  ...
  const displayDirectors = crew.directors.length > 0 
    ? crew.directors 
    : ["No director information"];
    
  const displayWriters = crew.writers.length > 0 
    ? crew.writers 
      ...
      <div className="flex gap-16 justify-center">
        <div>
          ...
          <MetaInfoDiv
            ...
            value={
              <div>
                {isLoading ? "Loading..." : displayDirectors.slice(0, 3).map((director, i) => (
                  <div key={i}>{director}</div>
                ))}
              </div>
            }
          />
          <MetaInfoDiv
            name={"Country"}
            value={media.origin_country?.[0] ? countryMap[media.origin_country[0]] : "N/A"}
          />
        </div>
        ...
        <div>
          {media.runtime ? (
            ...
            <MetaInfoDiv
              name={"Duration"}
              value={`${media.seasons?.length || 0} Seasons, ${totalEpisodes} Episodes`}
            />
          )}
          <MetaInfoDiv
            name="Writer"
            value={
              <div>
                {isLoading ? "Loading..." : displayWriters.slice(0, 3).map((writer, i) => (
                  <div key={i} className="flex">{writer}</div>
                ))}
              </div>
            }
          />
          <MetaInfoDiv
            name={"Language"}
            value={languageMap[media.original_language] || media.original_language}
          />
          <MetaInfoDiv
            name={"Genre"}
            value={
