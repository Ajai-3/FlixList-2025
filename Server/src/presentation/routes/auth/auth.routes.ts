...
router.get(
  "/bootstrap",
  limits.session,
  sessionAuth.bootstrap as RequestHandler
);
router.get(
  "/refresh-token",
  limits.session,
  sessionAuth.refreshToken as RequestHandler
);
...
router.post(
  "/user/login",
  limits.userLogin,
  userAuth.userLogin as RequestHandler
);
router.post(
  "/user/verify-otp",
  limits.userVerifyOtp,
  userAuth.verifyOtp as RequestHandler
);
router.post(
  "/user/resend-otp",
  limits.userResendOtp,
  userAuth.resendOtp as RequestHandler
);
router.post(
  "/user/register",
  limits.userRegister,
  userAuth.userRegister as RequestHandler
);
router.post(
  "/user/forgot-password",
  limits.userForgotPasswordLimit,
  userAuth.forgotPassword as RequestHandler
);
router.post(
  "/user/reset-password",
  limits.userResetPasswordLimit,
  userAuth.resetPassword as RequestHandler
);
router.post(
  "/user/logout",
  limits.userLogout,
  userAuth.userLogout as RequestHandler
);
...
router.post(
  "/admin/login",
  limits.adminLogin,
  adminAuth.adminLogin as RequestHandler
);
router.post(
  "/admin/logout",
  limits.adminLogout,
  adminAuth.adminLogout as RequestHandler
);
